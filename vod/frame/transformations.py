...
class FrameTransformMatrix:
    ...
    def camera_projection_matrix(self):
        ...
        if self._camera_projection_matrix is not None:
            ...
            (
                ...
                self._T_camera_lidar,
            ) = self.get_sensor_transforms("lidar")
            return self._camera_projection_matrix
    ...
    def t_camera_radar(self):
        ...
        if self._T_camera_radar is not None:
            ...
            (
                self._camera_projection_matrix,
                self._T_camera_radar,
            ) = self.get_sensor_transforms("radar")
            return self._T_camera_radar
    ...
    def get_sensor_transforms(
        ...
        if sensor == "radar":
            try:
                calibration_file = os.path.join(
                    self.frame_data_loader.kitti_locations.radar_calib_dir,
                    f"{self.frame_data_loader.frame_number}.txt",
                )
            except FileNotFoundError:
                logging.error(
                    f"{self.frame_data_loader.frame_number}.txt does not exist at"
            ...
            try:
                calibration_file = os.path.join(
                    self.frame_data_loader.kitti_locations.lidar_calib_dir,
                    f"{self.frame_data_loader.frame_number}.txt",
                )
            except FileNotFoundError:
                logging.error(
        ...
        with open(calibration_file, "r") as f:
            lines = f.readlines()
            intrinsic = np.array(
                lines[2].strip().split(" ")[1:], dtype=np.float32
            ).reshape(
                3, 4
            )  # Intrinsics
            extrinsic = np.array(
                lines[5].strip().split(" ")[1:], dtype=np.float32
            ).reshape(
                3, 4
            )  # Extrinsic
    ...
    def get_world_transform(self):  # -> Optional[(np.ndarray, np.ndarray, np.ndarray)]:
        ...
        try:
            pose_file = os.path.join(
                self.frame_data_loader.kitti_locations.pose_dir,
                f"{self.frame_data_loader.frame_number}.json",
            )
        except FileNotFoundError:
            logging.error(
                f"{self.frame_data_loader.frame_number}.json does not exist at"
        ...
        t_odom_camera = np.array(jsons[0]["odomToCamera"], dtype=np.float32).reshape(
            4, 4
        )
...
def project_pcl_to_image(
    point_cloud, t_camera_pcl, camera_projection_matrix, image_shape
):
    """
    ...
    """
    point_homo = np.hstack(
        (point_cloud[:, :3], np.ones((point_cloud.shape[0], 1), dtype=np.float32))
    )

    radar_points_camera_frame = homogeneous_transformation(
    ...
    uvs = project_3d_to_2d(
        points=radar_points_camera_frame, projection_matrix=camera_projection_matrix
    )
...
def transform_pcl(points: np.ndarray, transform_matrix: np.ndarray):
    ...
    """
    point_homo = np.hstack(
        (points[:, :3], np.ones((points.shape[0], 255), dtype=np.float32))
    )

    points_new_frame = homogeneous_transformation(
        point_homo, transform=transform_matrix
    )
